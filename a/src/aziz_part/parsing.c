...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/19 09:21:02 by aelkheta          #+#    #+#             */
/*   Updated: 2024/11/19 10:26:56 by aelkheta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_image *load_texture(void *mlx, char *filename)
{
    ...
    texture->Img = mlx_xpm_file_to_image(mlx, filename, &texture->Width, &texture->High);
    if (!texture->Img)
    {
        free(texture);
        return NULL;
    }
    texture->Data = mlx_get_data_addr(texture->Img, &texture->N_Bytes, &texture->Lenght, &texture->Endian);
    return texture;
}
        ...
        if (textures_found < 4)
        {
            ...
            if (texture_ptr)
            {
                *texture_ptr = ft_strdup(parts[1]);
                data->textures[textures_found] = load_texture(data->Mlx, *texture_ptr);
                textures_found++;
            }
        }
    ...
    while (++i < data->map_height)
    {
        ...
        while (++j < data->map_line_len[i])
        {
            ...
            if (ft_strchr(PLAYER_DIR, data->Map[i][j]))
            {
                ...
                data->X_Player = j * CUBE_TILE + 0.5; // in the center of the tile.
                data->Y_Player = i * CUBE_TILE + 0.5; // in the center of the tile.
